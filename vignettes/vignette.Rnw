...
We note that the algorithm is numerically sensitive to some degree, and some settings of initial guess may fail to converge. Similarly, intermediate steps may generate warning messages from methods in \texttt{deSolve}. 
