...
<<>>=
set.seed(100)
num.patients <- 100  #recommend at least 100
...
relTol = .00001 #set relative convergence criteria, in practice use smaller
...
Finally, we can print and compare the estimates and likelihood values returned by each method:
<<eval=TRUE, tidy=FALSE>>=
EMrun$betaEstimate
