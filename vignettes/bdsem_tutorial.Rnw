...
<<>>=
nSims = 5   # further increase number of simulations for more precise estimates
...
numSims = 5 #in practice, increase number of MC simulations
...
<<eval=FALSE, tidy=FALSE>>=
gridLength = 16 #can set to 32 for more accuracy
s1.seq <- exp(2*pi*1i*seq(from = 0, to = (gridLength-1))/gridLength)
s2.seq <- exp(2*pi*1i*seq(from = 0, to = (gridLength-1))/gridLength)

#perturb around true values for initial guess; in practice increase variance:
betaInit <- as.vector(rmvnorm(1, mean = betas, sigma = diag(.01*abs(betas) )))

relTol = .000001 #set relative convergence criteria, in practice use smaller
...
optimRun <- FFT.pat.optim(betaInit, t.pat, num.patients, PATIENTDATA, 
                            patients.design, s1.seq, s2.seq, relTol, max=2000, hess=FALSE )  
@

Finally, we can print and compare the converged estimates and log-likelihood values returned by each method. See Figure 4 in the main paper for results comparing EM and Optim over many trials.
