...
These empirical transition probabilities via simulation serve as a ground truth benchmark for comparison, and are computed using \texttt{tp.MC}. The results are stored in the same format as returned by \texttt{getTrans.timeList}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
FFT.rep = 2  \hlcom{#number of repetitions}
...
Finally, we include some example plotting code to produce boxplots that clearly illustrate the advantage in using our method to compute transition probabilities. This code is not evaluated here, but should be used after increasing the number of repetitions in the above experiments.
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
birthplot = \hlkwd{boxplot}(xaxt = \hlstr{'n'}, cex.lab = 1.3, ylab = \hlstr{"Estimate"}, 
                    ylim = \hlkwd{c}(.02,.127), FFT.params[[1]][1,], FM.params[[1]][1,], 
                    FFT.params[[2]][1,], FM.params[[2]][1,], FFT.params[[3]][1,], 
                    FM.params[[3]][1,], col = \hlkwd{c}(0,\hlstr{"grey"},0,\hlstr{"grey"},0,\hlstr{"grey"}), 
                    main = \hlstr{"Birth rate estimate"},cex.axis = 1.5, cex.main = 2.0, boxwex = .9)                                                                                                                                           
...
deathplot = \hlkwd{boxplot}(xaxt = \hlstr{"n"}, cex.lab = 2, xlab = \hlstr{"Time Interval Length"}, 
                    ylim = \hlkwd{c}(.04,.202), FFT.params[[1]][3,], FM.params[[1]][3,], 
                    FFT.params[[2]][3,], FM.params[[2]][3,],  FFT.params[[3]][3,], 
                    FM.params[[3]][3,], col = \hlkwd{c}(0,\hlstr{"grey"},0,\hlstr{"grey"},0,\hlstr{"grey"}), 
                    main = \hlstr{"Death rate estimate"}, cex.main = 2.0, boxwex = .9, cex.axis = 1.5)                                                                                                                                           
\hlkwd{axis}(1, at=\hlkwd{c}(1.5,3.5,5.5), labels=tList, cex.axis = 1.5)
...
shiftplot = \hlkwd{boxplot}(xaxt = \hlstr{'n'}, ylim = \hlkwd{c}(0,.078), FFT.params[[1]][2,], FM.params[[1]][2,], 
                    FFT.params[[2]][2,], FM.params[[2]][2,], FFT.params[[3]][2,], FM.params[[3]][2,], 
                    col = \hlkwd{c}(0,\hlstr{"grey"},0,\hlstr{"grey"},0,\hlstr{"grey"}), main = \hlstr{"Shift rate estimate"},
...
\hlkwd{legend}(\hlstr{"bottomright"},legend=\hlkwd{c}(\hlstr{"FFT"}, \hlstr{"FM"}), pch = \hlkwd{c}(0,15), pt.cex = 1.7, cex = 1.2, col=\hlkwd{c}(1, \hlstr{"grey"}))
...
numSims = 3 \hlcom{#in practice, may increase number of MC simulations}
...
\hlkwd{for}(i in 1:\hlkwd{length}(tList))\{
  ...
  lMC[,i] <- \hlkwd{apply}(results,1,quantile, probs = .025 )
  uMC[,i] <- \hlkwd{apply}(results, 1, quantile, probs = .975)
\}

\hlcom{#these are lists of matrices of restricted means given ending count, list index by end time in tlist}
...
\hlkwd{for}(i in 1:\hlkwd{length}(tList))\{
  eventsFFT[,i] <- \hlkwd{c}( \hlkwd{sum}(bmeans[[i]]), \hlkwd{sum}(smeans[[i]]), \hlkwd{sum}(dmeans[[i]]), -\hlkwd{sum}(ptimes[[i]]) )
\}

\hlcom{#pdf('restrictedMoments.pdf')}
\hlkwd{par}(mfrow=\hlkwd{c}(2,2))
\hlkwd{plot}(tList, eventsFFT[1,], pch = 1, col = 2, ylim = \hlkwd{c}(0,7.1), 
     ylab = \hlstr{"Expectation"}, xlab = \hlstr{"Observation Interval Length"}, main = \hlstr{"Expected Births"})
...
\hlkwd{plot}(tList, eventsFFT[2,], pch = 1, col = 2,ylim = \hlkwd{c}(0,1.7), 
     ylab = \hlstr{"Expectation"}, xlab = \hlstr{"Observation Interval Length"}, main = \hlstr{"Expected Shifts"})
...
\hlkwd{plot}(tList, eventsFFT[3,], pch = 1, col = 2, ylim = \hlkwd{c}(0,5.1), 
     ylab = \hlstr{"Expectation"}, xlab = \hlstr{"Observation Interval Length"}, main = \hlstr{"Expected Deaths"})
...
\hlkwd{plot}(tList, eventsFFT[4,], pch = 1, col = 2, ylim = \hlkwd{c}(0,65), 
     ylab = \hlstr{"Expectation"}, xlab = \hlstr{"Observation Interval Length"}, main = \hlstr{"Expected Particle Time"})
...
{\centering \includegraphics[width=\maxwidth]{figure/minimal-unnamed-chunk-9} 
...
\begin{verbatim}
##          [,1]    [,2]     [,3]    [,4]     [,5]
## [1,] 0.019530 0.38209 0.396176 0.00666 0.364480
## [2,] 0.005862 0.03464 0.006562 0.01176 0.005262
## [3,] 0.059600 0.19124 0.318432 0.04935 0.187961
\end{verbatim}
\begin{alltt}
\hlkwd{apply}(patients.rates, 1, mean)
\end{alltt}
\begin{verbatim}
## [1] 0.11037 0.02494 0.13883
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\begin{alltt}
t.pat <- 0.2
\hlcom{# apply(patients.rates,1,mean) #this line displays the average lam, v, mu given the fake}
\hlcom{# patients}
...
relTol = .0000001 \hlcom{#relative convergence criteria}
